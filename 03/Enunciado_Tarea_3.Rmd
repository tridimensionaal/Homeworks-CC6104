...
> En una regresión lineal simple el parámetro $beta_{1}$ corresponde a la pendiente de la recta y representa el cambio en y cuando variamos el valor de x. Mientras mayor sea la magnitud de este parámetro, más fuerte será la relación lineal. Si beta_{1} se parece a cero, entonces habrá una independencia lineal entre las variables, siendo la variable categórica x poco significativa para la variable de interés y. Dado que estamos considerando que x corresponde a una variable categórica, esta variable puede tomar un valor de un número limitado de valores, esto genera que el coeficiente ponderado por el valor de la categoría posea una relevancia diferente para Y dependiendo del valor asignado a la categoría. El coeficiente entonces tendrá una significancia diferente según pertenezca a una categoría o a otra.
...
Inicialmente, para construir la funcion `lm_reg` se crearon dos funciones auxiliares `get_vector_B` y `remove_name` que se utilizarán en la función `lm_reg` 
```{r}
...
Con las funciones `lm_reg` y `plot_reg`, se procederá a leer el data set `insurance` y preparar dicho data set que crear las regresiones respectivas y sus plots.
```{r}
...
Dado el data set arreglado que posee solo fumadores
```{r}
