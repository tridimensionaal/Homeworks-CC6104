...
$300 + 90*1= 390$, luego del segundo semestre habrán $300 + 90*2 = 480$

De esta manera, al final de la carrera (suponiendo una duración de 12 semestres) habrán $300 + 90*12 = 1.380$ fanáticos del animé.
...
> En una regresión lineal simple el parámetro beta_{1} corresponde a la pendiente de la recta y representa el cambio en y cuando variamos el valor de x, que en este caso va a corresponder a una variable categórica. Mientras mayor sea la magnitud de este parámetro, más fuerte será la relación lineal. Si beta_{1} se parece a cero, entonces habrá una independencia lineal entre las variables, siendo la variable categórica x poco significativa para la variable de interés y. 
...
> Nos dice que las eecuaciones de los parámetros corresponden a estimadores de máxima verosimilitud, se sabe que estos estimadores poseen ciertas propiedades, son consistentes, son asíntoticamente normales, es decir, que la sampling distribution de los estimadores comienzan a converger a una gaussiana lo cual nos permite calcular intervalos de confianza sobre los estimadores, se podría generar entonces un idc para los parámetros del modelo. Esto permite tener información acerca de la variabilidad de la estimación. Se pueden realizar test de hipótesis, para testear significancia por ejemplo.
...
> Minimizar el error cuadrático es equivalente a maximizar la verosimilitud...
...
> En el primer estudio se aprecia la existencia de una correlación entre el consumo de café y el padecer un cáncer pulmonar, sin embargo el segundo estudio demuestra que si bien existe una correlación entre los fenómenos descritos esta no corresponde a una relación causal y que en realidad existe una tercera variable implicada, la cuál corresponde a ser fumador y explica la existencia de la correlación.
...
# Cargar Dataset
library(readr)
insurance <- read_csv("C:/Users/camil/Homeworks-CC6104/03/insurance.csv")

insurance$smokers <- ifelse(insurance$smoker == 'yes', 1, 0)
#Separar dataset en fumadores y no fumadores
#Fumadores
df_f <- insurance[insurance$smoker == 'yes', ]
#No fumadores
df_nof <- insurance[insurance$smoker == 'no', ]

#Pasar variable categórica a variable dummy en una nueva columna llamada smokers
#Fumador = 1, No fumador = 0
df_f$smokers <- ifelse(df_f$smoker == 'yes', 1, 0)
df_nof$smokers <- ifelse(df_nof$smoker == 'no', 0, 1)

#Visualizar como quedan los datasets
#view(df_f)
#view(df_nof)

# Ver correlación entre las variables numéricas y  la variable de interés charges
...
cor(insurance$smokers,insurance$charges)

# Para el dataset de fumadores 
cor(df_f$age,df_f$charges)
cor(df_f$bmi,df_f$charges)
cor(df_f$children,df_f$charges)
cor(df_f$smokers,df_f$charges) # Posee desviación estándar 0
# Para el dataset de no fumadores 
cor(df_nof$age,df_nof$charges)
cor(df_nof$bmi,df_nof$charges)
cor(df_nof$children,df_nof$charges)
cor(df_nof$smokers,df_nof$charges) # Posee desviación estándar 0

# Para el dataser completo, la variable más correlacionada con charges corresponde a la variable smokers 
# Para el dataset de fumadores la variable que está más correlacionada es la de bmi o índice de masa corporal
# Para el dataset de no fumadores, la variable que está más correlacionada es la de age (edad)


# Se escoge la variable smokers, por ser la que posee la correlación más alta con la variable de interés en el dataset completo
reg_simple <- lm(charges ~ smokers, data = insurance)
reg_simple
# Se escoge la variable bmi, por ser la que posee la correlación más alta con la variable de interés para el dataset de fumadores 
reg_simple_f <- lm(charges ~ bmi, data = df_f)
reg_simple_f
# Se escoge la variable age, por ser la que posee la correlación más alta con la variable de interés para el dataset de no fumadores 
reg_simple_nof <- lm(charges ~ age, data = df_nof)
reg_simple_nof 
...
summary(reg_simple)
summary(reg_simple_f)
summary(reg_simple_nof)
sum.reg_simple <- summary(reg_simple)
sum.reg_simple$r.squared
SSE1 <- sum(reg_simple$residuals^2)
SST1 <- sum((insurance$smokers - mean(insurance$smokers))^2)
SSM1 <- sum((reg_simple$fitted.values - mean(insurance$smokers))^2)
SSM1/SST1
1-SSE1/SST1


sum.reg_simple_f <- summary(reg_simple_f)
sum.reg_simple_f$r.squared
SSE2 <- sum(reg_simple_f$residuals^2)
SST2 <- sum((df_f$bmi - mean(df_f$bmi))^2)
SSM2 <- sum((reg_simple_f$fitted.values - mean(df_f$bmi))^2)
SSM2/SST2
1-SSE2/SST2

sum.reg_simple_nof <- summary(reg_simple_nof)
sum.reg_simple_nof$r.squared
SSE3 <- sum(reg_simple_nof$residuals^2)
SST3 <- sum((df_nof$age - mean(df_nof$age))^2)
SSM3 <- sum((reg_simple_nof$fitted.values - mean(df_nof$age))^2)
SSM3/SST3
1-SSE3/SST3

# Grafique el scatterplot de los datos y la linea que ajusta a la regresión lineal obtenida.

attach(insurance)
plot(insurance$smokers, insurance$charges, main="Scatterplot",
   xlab="Smokers ", ylab="charges ", pch=19)
abline(reg_simple, col = "orange", lwd = 3)

attach(df_f)
plot(df_f$bmi, df_f$charges, main="Scatterplot",
   xlab="Índice de masa corporal ", ylab="charges ", pch=19)
abline(reg_simple_f, col = "orange", lwd = 3)

attach(df_nof)
plot(df_nof$age, df_nof$charges, main="Scatterplot",
   xlab="Age", ylab="charges ", pch=19)
abline(reg_simple_nof, data = df_nof, col = "orange", lwd = 3)
...
```{r}
# PARTE II
#Entrene un modelo lineal multivariable escogiendo dos variables numéricas que posean la mayor relación con `charges`.

reg_mult <- lm(charges ~ smokers + age , data = insurance)
reg_mult

reg_mult_f <- lm(charges ~ bmi + age , data = df_f)
reg_mult_f

reg_mult_nof <- lm(charges ~ age + children , data = df_nof)
reg_mult_nof
#Estudie si el modelo multivariable posee mejor desempeño que el modelo simple y comente los resultados. ¿Es recomendable la utilización de los modelos creados para la predicción de nuevas entradas?. Para este análisis puede utilizar los valores de test de hipótesis entregados por el comando `lm()`, ya que esto le servirá para observar si la regresión lineal es significativa.

summary(reg_mult)
summary(reg_mult_f)
summary(reg_mult_nof)
