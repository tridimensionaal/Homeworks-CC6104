...
```
```{r}
z.test.multiple_testing <- function(data1, data2, sigma1, sigma2, test=10){
  ...
  p_values <- c()
  for (i in 1:test){
    samp_data1 <- sample(1:n1, 100)
    samp_data2 <- sample(1:n2, 100)
    
    p <- z_test(data1=data1[samp_data1, ], data2=data2[samp_data2, ], sigma1=sigma1, sigma2=sigma2, mu.Ha=0, test.type = 'two-sided', verbose=FALSE)
    
    p_values <- append(p_values, p*test)
  }
  return(p_values)
}
```
...
```{r}
grad_v_master <- z.test.multiple_testing(data1 = grad, data2 = master, sigma1 = 28180, sigma2 = 20160)
any(grad_v_master < 0.05)
```


```{r}
grad_v_phd <- z.test.multiple_testing(data1 = grad, data2 = phd, sigma1 = 28180, sigma2 = 20615)
any(grad_v_phd < 0.05)
min(grad_v_phd)
```
...
plot_probs <- function(n, alpha, bonfe = FALSE){
  ...
  plot <- plot + 
    labs(x="Valor m (cantidad experimentos)", y="Probabilidad de obtener \n al menos un experimento exitoso", title="Probabilidades del evento")
...
```{r}
hola <- plot_probs(500, 0.05, bonfe = TRUE)
plot(hola)
```
